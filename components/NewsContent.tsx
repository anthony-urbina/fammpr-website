...
export default function NewsContent({ articles }: Props) {
  ...
  return (
    <div style={{ background: "var(--color-bg)" }}>
      {/* Page header */}
      <div className="relative pt-32 pb-16 overflow-hidden" style={{ background: "var(--color-surface)" }}>
        ...
        <div className="relative max-w-6xl mx-auto px-6">
          ...
          <h1 className="font-display text-[clamp(3rem,8vw,7rem)] uppercase leading-none text-fg" style={{ fontWeight: 900 }}>
            {n.heading}
          </h1>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-6 py-16">
        {articles.length === 0 ? (
          <p className="text-fg-muted text-center py-24">{n.empty}</p>
        ) : (
                ...
                <div className="flex-1">
                  <div className="flex flex-wrap items-center gap-3 mb-3">
                    ...
                    {article.author && (
                      <>
                        <span className="text-border">·</span>
                        <span className="text-xs text-fg-muted">{article.author}</span>
                      </>
                    )}
                  </div>
                  <h2
